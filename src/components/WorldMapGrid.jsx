...
const regionSets = {
    northAmerica: new Set(northAmerica),
    southAmerica: new Set(southAmerica),
...
const getRegionClass = (idx) => {
    if (regionSets.northAmerica.has(idx)) return "north-america";
    if (regionSets.southAmerica.has(idx)) return "south-america";
    if (regionSets.europe.has(idx)) return "europe";
    if (regionSets.africa.has(idx)) return "africa";
    if (regionSets.asia.has(idx)) return "asia";
    if (regionSets.australia.has(idx)) return "australia";
    return "";
};
...
const WorldMapGrid = () => {
    const cells = useMemo(() => {
        ...
        for (let i = 0; i < 35 * 55; i++) {
            const idx = i + 1;
            const region = getRegionClass(idx);
            items.push(
                <div
                    className={`cell${region ? ` region ${region}` : ""}`}
                    key={i}
                />
            );
        }
    ...
    return (
        <div className="world-map-wrapper">
            <div className="world-map-grid">{cells}</div>
        </div>
    );
};
